...
describe('CayenneLpp', () => {
  ...
  it('parses LPP_GPS', () => {
    ...
    const result = CayenneLpp.parse(bytes)
    expect(result[0].value.latitude).toBeCloseTo(51.2345, 3)
    expect(result[0].value.longitude).toBeCloseTo(4.2345, 3)
    expect(result[0].value.altitude).toBeCloseTo(100.0, 1)
  })
