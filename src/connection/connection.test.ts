...
describe('Connection', () => {
    ...
    describe('request helpers', () => {
        ...
        it('serializes concurrent message sync requests so each response reaches one caller', async () => {
            const connection = new TestConnection();

            const first = connection.syncNextMessage();
            const second = connection.syncNextMessage();

            await vi.waitFor(() => expect(connection.sentFrames).toHaveLength(1));

            connection.dispatch(makeContactMessageFrame('first'));
            await expect(first).resolves.toMatchObject({ contactMessage: { text: 'first' } });
            await vi.waitFor(() => expect(connection.sentFrames).toHaveLength(2));

            connection.dispatch(makeChannelMessageFrame('second'));
            await expect(second).resolves.toMatchObject({ channelMessage: { text: 'second' } });
        });

        it('continues the message sync queue after a command fails', async () => {
            const connection = new TestConnection();
            const error = new Error('write failed');
            vi.spyOn(connection, 'sendCommandSyncNextMessage')
            ...
            await vi.waitFor(() => {
                expect(connection.sendCommandSyncNextMessage).toHaveBeenCalledTimes(2);
            });

            connection.dispatch(makeContactMessageFrame('recovered'));
            await expect(second).resolves.toMatchObject({ contactMessage: { text: 'recovered' } });
        });
