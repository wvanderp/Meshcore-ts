...
describe('SerialConnection', () => {
    ...
    it('logs a processing failure for a single frame', async () => {
    ...
    it('resynchronizes after an impossible frame length', async () => {
        const connection = new TestSerialConnection();
        ...
        expect(connection.receivedFrames).toEqual([Uint8Array.of(0x42)]);
        expect(connection.readBuffer).toEqual([]);
    });

    it('continues with the next complete frame after a frame handler fails', async () => {
        const connection = new ThrowingSerialConnection();
        const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
