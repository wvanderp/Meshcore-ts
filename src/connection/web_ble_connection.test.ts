...
type ValueChangedHandler = (event: { target: { value: DataView } }) => void;
...
function createBleFixture() {
    ...
    return {
        ...
        emitValueChangedView: (value: DataView) => {
            valueChangedHandler?.({ target: { value } });
        },
        gattServer,
        rxCharacteristic,
        service,
...
describe('WebBleConnection', () => {
    ...
    it('passes only the notified DataView bytes to the frame parser', async () => {
        const fixture = createBleFixture();
        vi.spyOn(WebBleConnection.prototype, 'deviceQuery').mockResolvedValue({} as never);

        setNavigator({
            bluetooth: {
                requestDevice: vi.fn().mockResolvedValue(fixture.bleDevice),
            },
        });

        const connection = await WebBleConnection.open();
        const frameSpy = vi.spyOn(connection!, 'onFrameReceived');
        await vi.waitFor(() => expect(fixture.txCharacteristic.startNotifications).toHaveBeenCalled());

        const backingBytes = Uint8Array.of(0xee, 0xaa, 0xbb, 0xff);
        fixture.emitValueChangedView(new DataView(backingBytes.buffer, 1, 2));
